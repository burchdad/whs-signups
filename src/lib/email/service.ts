...
export async function sendSignupEmails(input: { signup: Signup; event: VolunteerEvent; slot: VolunteerSlot; cancellationToken: string; notificationEmails?: string[] }) {
  ...
  const recipients = notificationRecipients(input.event, input.notificationEmails).filter((email) => email !== input.signup.email.toLowerCase());
...
export async function sendCancellationEmails(input: { signup: Signup; event: VolunteerEvent; slot: VolunteerSlot; notificationEmails?: string[] }) {
...
export async function sendBoosterClubEmails(input: BoosterClubSignupInput & { programName: string; signupId?: string; notificationEmails?: string[] }) {
  ...
  const html = emailLayout(subject, "Your Booster Club signup was received.", `<h1 style="margin-top:0;color:#43110e">Welcome to the team behind the team</h1><p>Hi ${escapeHtml(input.firstName)},</p><p>We received your signup for <strong>${escapeHtml(input.programName)}</strong>.</p><p><strong>Programs:</strong> ${escapeHtml(sports)}<br><strong>Booster item:</strong> ${escapeHtml(input.gearPreference)}</p>${button("Visit Booster Club", appUrl("/booster-club"))}`);
  ...
  const recipients = notificationRecipients(undefined, input.notificationEmails).filter((email) => email !== input.email.toLowerCase());
  const owner = await send({ to: recipients, subject: `New ${input.programName} signup`, text: `${input.firstName} ${input.lastName} joined ${input.programName}.\nPrograms: ${sports}\nPhone: ${input.phone}\nEmail: ${input.email}`, html: emailLayout("New Booster Club signup", `${input.firstName} ${input.lastName} joined.`, `<h1 style="margin-top:0;color:#43110e">New ${escapeHtml(input.programName)} signup</h1><p><strong>${escapeHtml(input.firstName)} ${escapeHtml(input.lastName)}</strong> joined ${escapeHtml(input.programName)}.</p><p><strong>Programs:</strong> ${escapeHtml(sports)}<br><strong>Email:</strong> <a href="mailto:${escapeHtml(input.email)}">${escapeHtml(input.email)}</a><br><strong>Phone:</strong> ${escapeHtml(input.phone)}<br><strong>Open to volunteering:</strong> ${escapeHtml(input.openToVolunteering)}<br><strong>Interested in sponsoring:</strong> ${escapeHtml(input.interestedInSponsoring)}</p>${button("Open Booster signups", appUrl("/admin/booster-club"))}`), replyTo: input.email, idempotencyKey: `booster-owner-${key}` });
  return { volunteer, owner };
}
