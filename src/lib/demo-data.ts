...
export const events: VolunteerEvent[] = eventSeeds.map((seed, index) => {
  ...
  return {
    ...
    slots: [
      ...
      { id: slotId(index, 9), eventId, name: "Adult Volunteer", category: "Adult Volunteers", shiftStart: centralIso(seed.date, seed.start), shiftEnd: centralIso(seed.date, seed.start, 120), capacity: 2, filled: 0, isOpen: true, isVisible: true, sortOrder: 100, instructions: "Adult shift runs 2 hours from the event start time." },
    ],
  };
});
